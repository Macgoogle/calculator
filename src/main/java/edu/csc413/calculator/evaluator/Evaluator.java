...
public class Evaluator {
  ...
  private final String delimiters = Operator.getALlOperators();
      ...
      if ( !( expressionToken = this.expressionTokenizer.nextToken() ).equals( " " )) {
        ...
        if ( Operand.check( expressionToken )) {
          ...
          while (!operatorStack.isEmpty()&&operatorStack.peek().priority() >= newOperator.priority() ) {
            ...
            Operand result1 = operatorFromStack.execute( operandOne, operandTwo );
  ...
  public void calculate(Operator operator){
    while(operatorStack.size()>0){
      ...
      Operand operandTwo = operandStack.pop();
      Operand operandOne = operandStack.pop();
      operandStack.push(currentOperator.execute(operandOne,operandTwo));
    }
